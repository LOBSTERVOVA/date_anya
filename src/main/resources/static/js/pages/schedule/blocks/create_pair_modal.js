import {formatLectFio} from '../utils.js'
...
export async function savePairModal() {
    ...
    if (roomName) {
        let roomUuid = $('#pair-room-search').attr('data-selected-room-uuid') || '';
        if (roomUuid) {
            roomPayload = { uuid: roomUuid };
        } else {
            // Ищем аудиторию по названию в свободных
            const room = window.freeRooms.find(r => (r.name || r.title) === roomName);
            if (room) {
                roomPayload = { uuid: room.uuid };
            }
        }
    }

    // Формируем payload для создания/обновления пар
    const basePayload = {
        name: subjectName,
        room: roomPayload,
        pairOrder: pairOrder,
        subject: subjectUuid ? { uuid: subjectUuid } : null,
        lecturers: window.selectedLecturers.map(l => ({ uuid: l.uuid }))
    };
    ...
    try {
        ...
        for (const groupUuid of selectedGroupUuids) {
            ...
            // Ищем существующую пару для этой группы
...
export function getFreeLecturersForTime() {
    ...
    Object.keys(window.currentTableData).forEach(cellKey => {
...
export function getBusyRoomsForTime() {
    ...
    Object.keys(window.currentTableData).forEach(cellKey => {
        const [lectUuid, cellDayIdx, cellPairOrder] = cellKey.split('_');
        if (Number(cellDayIdx) === dayIdx && Number(cellPairOrder) === pairOrder) {
            const pairs = window.currentTableData[cellKey] || [];
            pairs.forEach(pair => {
