...
public class ClubService {
     ...
     * @param departmentUuids фильтр по кафедрам (null или пустой — все кафедры)
    ...
    public Mono<ClubPageDto> findAllByType(String type, int page, int size,
                                            String search, Set<UUID> departmentUuids,
                                            Integer dayOfWeek, String timeFrom, String timeTo) {
        long offset = (long) page * size;
        UUID[] deptArr = (departmentUuids != null && !departmentUuids.isEmpty())
                ? departmentUuids.toArray(new UUID[0]) : null;
        Mono<Long> totalMono = clubRepository.countByTypeWithFilters(type, search, deptArr, dayOfWeek, timeFrom, timeTo);
        Flux<ClubDto> contentFlux = clubRepository.findByTypeWithFilters(type, search, deptArr, dayOfWeek, timeFrom, timeTo, size, offset)
                .flatMap(this::enrichWithSchedules)
                .flatMap(this::toDto);
